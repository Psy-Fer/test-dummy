...
process minimap2 {
    ...
    script:
        """
        minimap2 --secondary=no --MD -ax map-ont -t 8 ${params.azureFileShare}/ref/${params.ref_genome} ${params.azureFileShare}/${params.reads} | samtools sort - > minimap2.sorted.bam
...
process clair3 {
    ...
    script:
        ...
        --bed_fn=${params.azureFileShare}/ref/${params.thal_regions} \
        --bam_fn=$bam \
...
process whatshap {
    ...
    script:
        ...
        whatshap phase \
        --sample $sample_id \
        --reference ${params.azureFileShare}/ref/${params.ref_genome} \
...
process sniffles2 {
    ...
    script:
        ...
        --sample-id $sample_id \
        --reference ${params.azureFileShare}/ref/${params.ref_genome} \
        --tandem-repeats ${params.azureFileShare}/ref/${params.tandem_repeat_bed} \
        --output-rnames \
        --minsvlen 20 \
        --input $bam \
        --vcf sniffles.vcf \
...
process postproccess {
    ...
    shell:
        ...
        --threads !{task.cpus} \
        --bam_fn !{bam} \
        --clair3_vcf_input clair3_whatshap_filtered.vcf.gz \
...
process publishfiles {
    ...
    input:
        ...
        path clair3_whatshap_filtered_vcf
        path clair3_whatshap_filtered_vcf_tbi
    ...
    script:
        ...
        cp $clair3_whatshap_filtered_vcf ${params.azureFileShare}}/$sample_id/$sample_id-$clair3_whatshap_filtered_vcf.name
        cp $clair3_whatshap_filtered_vcf_tbi ${params.azureFileShare}}/$sample_id/$sample_id-$clair3_whatshap_filtered_vcf_tbi.name
...
workflow {
    ...
    whatshap(sample_id, minimap2.out[0], minimap2.out[1], clair3.out)
    sniffles2(sample_id, whatshap.out[2], whatshap.out[3])
